  ...
  return (
    <Section title={`積分${layer.label ? ` — ${layer.label}` : ''} (${mine.length})`}>
      ...
      <div className="row wrap">
        ...
        <button onClick={() => setTool('integral')}>手で引く</button>
        ...
        <div className="row wrap">
          <button
            ...
            onClick={() => {
              ...
              notify(n ? `Delta の積分を ${n} 件取り込みました` : 'すべて取り込み済みです');
            }}
          >
            Delta の積分を取り込む ({fromDelta.length})
          </button>
        </div>
      )}
      <div className="row wrap">
        <label className="field" title="一番高いピークの何 % 以上を信号とみなすか">
          高さ
      ...
      <details className="sub">
        <summary>積分の見た目</summary>
        <div className="row wrap">
          <span title="範囲の両端を結ぶ直線を引いてから足します (Delta と同じ出し方)">
        ...
        <label className="field block">
          ...
          <select value={f.integralPlacement} onChange={(e) => set({ integralPlacement: e.target.value as FigureStyle['integralPlacement'] })}>
            <option value="curve">曲線の上 (Delta 風)</option>
            <option value="axis">軸の下にかぎ括弧で</option>
          </select>
        </label>
        <div className="grid2">
          <label className="field">
          ...
          <label className="field" title="一番大きい積分曲線の高さ (スペクトルの帯に対する割合)">
            曲線の高さ
    ...
    <tr className={selected ? 'selected' : ''} onClick={() => select({ kind: 'integral', id: x.id })}>
      ...
      <td>
        <NumberInput
          ...
          step={1}
          width={56}
          title="値を書き換えると、この積分を基準にしてほかをそろえます"
